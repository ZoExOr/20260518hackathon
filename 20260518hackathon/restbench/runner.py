...
def _format_multiagent_trace(trace) -> str:
    if trace is None:
        return ""
    data = trace.to_dict()
    advice = [
        f"{a.get('role')}:{a.get('summary', '')[:45]}"
        for a in data.get("advice", [])
    ]
    review = data.get("risk_review") or {}
    hard = data.get("deterministic_risk") or []
    drops = sum(len(r.get("dropped_actions", [])) for r in hard)
    adjusts = sum(len(r.get("adjusted_actions", [])) for r in hard)
    risk = "-"
    if hard:
        risk = hard[-1].get("supply_risk", {}).get("level", "-")
    return (
        f"llm_advice=[{'; '.join(advice) if advice else '-'}] "
        f"llm_risk={review.get('risk_score', '-')} "
        f"hard_risk={risk} hard_drop/adj={drops}/{adjusts}"
    )
...
def play_game(team_name: str, scenario: str = "baseline", seed: int = 42,
              ...
              multi_agent: bool = False, multi_agent_advisor=None
              ) -> dict:
    client = client or RestBenchClient()
    agent = Agent(params=params, regime=regime, multi_agent=multi_agent,
                  multi_agent_advisor=multi_agent_advisor)
    ...
    while status == "in_progress":
        ...
        if replay:
            extra = {}
            if agent.last_deterministic_risk:
                extra["deterministic_risk"] = agent.last_deterministic_risk
            if agent.last_multiagent_trace is not None:
                extra["multiagent"] = agent.last_multiagent_trace.to_dict()
            replay.log(scenario, seed, obs.day, obs.raw, submitted,
                       turn.get("day_result"), extra=extra)
        status = turn["status"]
        if verbose:
            print(
            ...
            trace_line = _format_multiagent_trace(agent.last_multiagent_trace)
            if trace_line:
                print(f"  multiagent {trace_line}")
